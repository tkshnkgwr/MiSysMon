...
## [2026-07-21]
### Fixed
- **画面絶対マウス座標基準のアンカー移動方式への刷新**: Win32 `GetCursorPos` による開始点アンカー方式を導入し、ドラッグ移動中にウィンドウがカーソルから離れて飛んでいく現象および位置の振動・分身現象を修正。マウスポインタへの完全な追従とリアルタイムなスナップ動作を実現。
- **マルチディスプレイ環境での設定画面表示位置指定**: 設定ウィンドウ (`ViewportBuilder`) にメインウィンドウ座標を基準とした `.with_position()` を明示的に設定し、メインウィンドウが存在するモニター上に設定画面が開くよう修正。
- **画面端オートスナップ機能の判定強化**: 吸着時に画面モニターを取得する Win32 API 呼び出しの参照点をウィンドウ左上から中心座標 `(center_x, center_y)` に変更し、画面右端・下端やマルチモニター境界でのスナップ失敗を防ぎました。
- **表示要素（IO/VER/日時）の重なり・幅不足の解消**: `calculate_width()` における各コンポーネント（CPU, MEM, NET, DISK, IO, VER, CLOCK）の必要幅計算を実際の描画サイズに合わせて正確化し、要素同士の重複を修正しました。

### Optimized
- **ウィンドウサイズ変更コマンドの最適化**: 毎フレーム無条件で呼び出されていた `ViewportCommand::InnerSize` を、直前の幅から変更が生じた時のみ呼び出すように最適化し、描画負荷とOSレベルの処理遅延を削減しました。
